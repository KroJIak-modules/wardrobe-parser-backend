...
class DedupServiceV2:
    ...
    def _candidate_score(self, left: dict, right: dict) -> tuple[float, list[str]]:
        ...
        left_vendor = self._norm(left.get("vendor"))
        right_vendor = self._norm(right.get("vendor"))
        if left_vendor and left_vendor == right_vendor:
            reasons.append("same_designer")
            ratio += 0.1
        if left.get("weight_grams") and right.get("weight_grams") and left["weight_grams"] == right["weight_grams"]:
    ...
    def list_candidates(self, *, limit: int, offset: int, payload_builder) -> dict:
        ...
        payloads = [payload_builder(product) for product in products if payload_builder(product).get("status") != "hidden"]
    ...
    def merge(self, *, primary_product_id: int, duplicate_product_id: int) -> int:
        ...
        for listing in [*primary_members, *duplicate_members]:
