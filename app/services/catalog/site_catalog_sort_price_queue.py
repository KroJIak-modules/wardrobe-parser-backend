...
_QUEUE_KEY = "catalog:site-sort-price:product-ids"
_SESSION_PENDING_IDS_KEY = "site_sort_price_pending_product_ids"
_SESSION_FLUSH_REGISTERED_KEY = "site_sort_price_pending_product_ids_registered"


class SiteCatalogSortPriceQueue:
    ...
    def enqueue_product_ids_after_commit(self, session: Session, product_ids: set[int] | list[int] | tuple[int, ...]) -> None:
        ...
        def _flush() -> None:
            ...
            try:
                ...
                LOGGER.exception("Failed to enqueue site sort price refresh for %s products", len(queued_ids))
    ...
    def pop_ready_batch(self, *, limit: int, debounce_sec: int) -> list[int]:
        ...
        for raw_value in raw_items:
            try:
                ...
                LOGGER.warning("Site sort price queue received non-integer item: %r", raw_value)
                continue
            if product_id <= 0 or product_id in seen:
                continue
            seen.add(product_id)
