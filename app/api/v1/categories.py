...
def create_category(payload: CategoryCreateRequest, db: Session = Depends(get_db), _: object = Depends(require_admin_access)):
...
def update_category(category_id: int, payload: CategoryUpdateRequest, db: Session = Depends(get_db), _: object = Depends(require_admin_access)):
    return CategoryTreeService(db).update_category(category_id, payload)


@router.delete("/categories/{category_id}")
def delete_category(category_id: int, db: Session = Depends(get_db), _: object = Depends(require_admin_access)):
    return CategoryTreeService(db).delete_category(category_id)


@router.post("/categories/{category_id}/keywords")
def add_category_keyword(category_id: int, payload: CategoryKeywordRequest, db: Session = Depends(get_db), _: object = Depends(require_admin_access)):
...
def get_manual_products(category_id: int, db: Session = Depends(get_db), _: object = Depends(require_admin_access)):
...
def search_manual_products(category_id: int, query: str = Query(min_length=1, max_length=255), limit: int = Query(default=3), db: Session = Depends(get_db), _: object = Depends(require_admin_access)):
...
def add_manual_product(category_id: int, payload: CategoryManualProductRequest, db: Session = Depends(get_db), _: object = Depends(require_admin_access)):
...
def remove_manual_product(category_id: int, product_id: int, db: Session = Depends(get_db), _: object = Depends(require_admin_access)):
